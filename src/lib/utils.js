...
module.exports = {
	...
	getElementAbsPos: function (elementId) {
	...
	handleScrollUp: function (scope) {
		var self = this;
		var fixedBarHeight = document.getElementById('fixedBarEle').clientHeight;
		var prevTop = self.getElementAbsPos("fixedBarEle").y;
		var nextTop = self.getElementAbsPos("monthBar-" + (scope.panelState)).y;
		var relativeHeight = (nextTop - prevTop);
		if (relativeHeight < 0) {
			scope.fixedMonthbar = document.getElementById('monthBar-' + (scope.panelState)).innerHTML;
			scope.panelState = scope.panelState + 1;
		}
	},
	handleScrollDown: function(scope){
		var self = this;
		scope.hiddenPanelState =  scope.panelState - 1;
		var hiddenPanelState =  self.getElementAbsPos("monthBar-" + (scope.hiddenPanelState)).y;
		var fixedBarHeight = document.getElementById('fixedBarEle').clientHeight;
		var prevTop = self.getElementAbsPos("fixedBarEle").y;
		var relativeHeight = prevTop - hiddenPanelState;
		if(relativeHeight + fixedBarHeight <= 0){
			scope.fixedMonthbar = document.getElementById('monthBar-' + (scope.hiddenPanelState - 1)).innerHTML;
			scope.panelState = scope.panelState -1;
		}
